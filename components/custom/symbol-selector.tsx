...
export function SymbolSelector({
  ...
  return (
    ...
    >
          ...
          {movement && (
            <span
              className={cn(
                ...
                movement.isUp ? 'text-neon-green' : 'text-neon-pink'
              )}
            >
      ...
      <SelectContent className="astral-glass border-neon-cyan/30">
        {Array.from(grouped.entries()).map(([submarket, { displayName, symbols: group }]) => (
          <SelectGroup key={submarket}>
            <SelectLabel className="text-neon-cyan font-semibold">{displayName}</SelectLabel>
            {group.map((symbol) => (
              <SelectItem
                key={symbol.underlying_symbol}
                value={symbol.underlying_symbol}
                className="hover:bg-neon-cyan/10 focus:bg-neon-cyan/20"
              >
